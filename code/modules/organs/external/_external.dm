...
/obj/item/organ/external/proc/createwound(var/type = CUT, var/damage, var/surgical)
	...
	if(!surgical && (type in list(CUT, PIERCE, BRUISE)) && damage > 30 && local_damage > 40) //START BEARHAMMER NONMODULAR EDIT: WHY ARE ARTERIES LIKE BALOONS? increased && damage to 30 from 15 and local damage to 40
...
/obj/item/organ/external/proc/droplimb(var/clean, var/disintegrate = DROPLIMB_EDGE, var/ignore_children, var/silent)
	...
	owner.unlock_achievement(new/datum/achievement/lose_limb())//Get an achievement for losing your limb!
...
/obj/item/organ/external/proc/fracture()
	//if(!config.bones_can_break) //Why the fuck was this ever a config?
	//	return
	if(robotic >= ORGAN_ROBOT)
	...
	if(owner)
		...
		if(can_feel_pain())
			if(prob(50))
				owner.agony_scream()
		if(break_sound)
			playsound(owner, break_sound, 100, 0)
	status |= ORGAN_BROKEN
	broken_description = "shattered"//,"fracture","hairline fracture")
